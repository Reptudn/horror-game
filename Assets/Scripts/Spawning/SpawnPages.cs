...
public class SpawnPages : MonoBehaviour
{
    ...
    void Spawn(){
        ...
        while(pagesSpawned < pagesAmount){
            ...
            if(pagesSpawned >= pagesAmount) break;
            ...
            if(spawnPoints[random].childCount == 0){
                ...
                pagesPrefabs[pageRand].transform.SetParent(spawnPoints[random].transform);
                pagesPrefabs[pageRand].transform.position = spawnPoints[random].transform.position;
                pagesSpawned++;
            }
